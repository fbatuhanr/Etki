...
function generateAccessToken(user) {
    ...
    return jsonwebtoken_1.default.sign({ userId: id, username: user.username }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: process.env.ACCESS_TOKEN_EXPIRATION });
}
function generateRefreshToken(user) {
    const id = user.userId instanceof mongoose_1.default.Types.ObjectId ? user.userId.toString() : user.userId;
    return jsonwebtoken_1.default.sign({ userId: id, username: user.username }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: process.env.REFRESH_TOKEN_EXPIRATION });
}
